...
const StudentSidebar = ({ studentName, courseName }) => {
  ...
  const navItems = [
    ...
    { href: '/student/results', label: 'Resultado', icon: LayoutDashboard },
    { href: '/student/exams', label: 'Exames', icon: BarChart3 },
    { href: '/student/documents', label: 'Documentos', icon: FileArchive },
    { href: '/student/matrix', label: 'Matriz', icon: FileArchive },
  ]

  const isActive = (path) => {
    return window.location.pathname === path || (path !== '/student/dashboard' && window.location.pathname.startsWith(path))
  }
  ...
  return (
    <aside className="w-72 bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100 p-6 flex flex-col fixed h-full shadow-2xl">
      <div className="mb-10 text-center">
        ...
        <h2 className="text-xl font-semibold">{studentName || 'Aluno'}</h2>
      ...
      <nav className="flex-grow space-y-3">
        {navItems.map((item) => (
          ...
          >
            <item.icon
              size={22}
              className={`transition-colors ${isActive(item.href) ? 'text-white' : 'text-slate-400 group-hover:text-sky-300'}`}
            />
