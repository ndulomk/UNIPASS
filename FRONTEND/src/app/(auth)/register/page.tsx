...
export default function RegisterPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allowedTypes = {
      ...
      bilhete: ["image/jpeg", "image/png"],
      vacine_card: ["application/pdf", "image/jpeg", "image/png"],
      foto: ["image/jpeg", "image/png"],
    };
    if (!allowedTypes[name as keyof typeof allowedTypes].includes(file.type)) {
      toast.error(`Formato inválido para ${name}. Use ${allowedTypes[name as keyof typeof allowedTypes].join(" ou ")}.`);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const missingFields = [];
    if (!formData.first_name) missingFields.push("Nome");
    if (!formData.last_name) missingFields.push("Sobrenome");
    if (!formData.email) missingFields.push("Email");
    if (!formData.password) missingFields.push("Senha");
    if (!formData.course_id) missingFields.push("Curso");
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full px-4 py-3 rounded-lg bg-white/10 focus:bg-white text-white focus:text-black focus:outline-none focus:ring-2 focus:ring-blue-400 border border-white/10"
                      required
                    >
