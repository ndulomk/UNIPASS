...
import { StudentExamResult, EnrollmentDetail } from '@/types'; // Added EnrollmentDetail
import { fetchStudentResults, fetchEnrollmentDetails } from '@/lib/api'; // Added fetchEnrollmentDetails
import Modal from '@/components/Modal'; // Ensure path is correct
...
const formatDate = (dateString?: string | null, includeTime = false) => {
    ...
    const options: Intl.DateTimeFormatOptions = { 
        day: '2-digit', 
        month: 'long', // Using 'long' for a more formal look
        year: 'numeric',
...
export default function StudentResultsPage() {
    ...
    const enrollmentId = params.enrollmentId ? parseInt(params.enrollmentId as string, 10) : null;

    const [results, setResults] = useState<StudentExamResult[]>([]);
    const [enrollmentDetails, setEnrollmentDetails] = useState<EnrollmentDetail | null>(null);
    ...
    useEffect(() => {
        if (enrollmentId) {
            ...
            setError(null);
            Promise.all([
                fetchStudentResults(enrollmentId),
    ...
    const getStatusPillClass = (grade: string) => {
    ...
    const getScoreColor = (score: number) => {
        if (score >= 9.5) return 'text-green-400'; // Aprovado (Angolan system)
        if (score >= 7) return 'text-yellow-400'; // Recuperação / Atenção
        return 'text-red-400'; // Reprovado
    };
    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4">
                ...
                <p className="text-xl text-blue-300">Carregando seus resultados...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-red-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <p className="text-red-300 text-xl mb-2">Oops! Algo deu errado.</p>
                <p className="text-gray-400 text-center mb-6">{error}</p>
                <button onClick={() => router.back()} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-medium transition-colors">Voltar</button>
            </div>
        );
    }
            ...
            <div className="max-w-5xl mx-auto">
                <header className="flex flex-col sm:flex-row justify-between items-center mb-10 pb-6 border-b border-gray-700">
                    ...
                    <button 
                        ...
                        className="mt-4 sm:mt-0 px-5 py-2.5 text-sm bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg transition-colors flex items-center gap-2"
                    >
                        ...
                        {results.map((result) => (
                            ...
                            >
                                ...
                                <div className="mt-auto pt-4 border-t border-gray-700 flex justify-between items-baseline">
                                     <p className="text-sm text-gray-400">Nota Final (0-20):</p>
                 ...
                 <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Meus Dados de Matrícula">
                    <div className="space-y-3 text-gray-300">
                        ...
                        <div className="p-4 bg-gray-700/50 rounded-lg">
                            ...
                            <p className={`font-medium ${enrollmentDetails.status === 'ativo' ? 'text-green-400' : 'text-yellow-400'}`}>{enrollmentDetails.status || 'N/A'}</p>
                        </div>
